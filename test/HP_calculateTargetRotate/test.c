...
void test_HP_calculateTargetRotateXY1() {
    HP_Status_t status = {0,0,0,100,0};
    HP_Leg_t leg = {0,0,0,10,0,0,0,0,0};

    HP_calculateTargetRotate(&status, &leg, -10, LEG_MODE_FLOATING);
...
void test_HP_calculateTargetRotateZ() {
    ...
    HP_calculateTargetRotate(&status, &leg, 0, LEG_MODE_FLOATING);

    TEST_ASSERT_EQUAL(-20, leg.tgt.z);

    leg.tgt = (Point3D_t) {0, 0, 0};
    HP_calculateTargetRotate(&status, &leg, 0, LEG_MODE_GROUNDED);
